...
fig, axs = plt.subplots(figsize=[12,5.5], nrows=2, ncols=3, constrained_layout=True)
...
for i, label in enumerate(('A', 'B')):
    ...
    axs[i,0].text(-0.15, 1.0, label, transform=axs[i,0].transAxes,
...
def task_test(task, n, s, lp, nonlin):
    tresholds= {
        'copy':   {'top_50':[0.09,0.09, 0.09] ,'select':[0.075,0.075, 0.075], 'top_25':[0.079,0.075, 0.08], 
                    'baseline':[0.09, 0.09, 0.09],'all':[1,1,1], 'top_20':[0.07,0.07,0.08], 'top_3':[0.03,0.03,0.03]},
        'psMNIST':{'top_50':[0.77,0.77,0.8],'top_25':[0.917,0.9005,0.907], 'top_10':[0.93,0.945,0.94],'top_20':[0.9225,0.905,0.92],
                   'acc_50':0.50,'all':[0,0,0], 'select':[0.93,0.945,0.94], 'baseline':[0.12, 0.12, 0.12], 'all':[0.12, 0.12, 0.12], 'top_3':[0.93, 0.948,0.93]},
        'PTB':    {'top_50':[1.64,1.67,1.64],'top_25':[1.612,1.625,1.615],'top_10':[1.60,1.595,1.6], 'top_20':[1.612,1.625,1.615],
                    'acc_50':[2.5,2.5], 'all':[5,5,5], 'baseline':[5,5,5], 'top_3':[1.594, 1.592, 1.59]} #1.67
    }
...
#                 axs[row,col].plot(LEs_array[:, index[0], index[1], 0], color='grey', label=label);
    ...
    for k in range(3):
        axs[0,i].plot(np.arange(5), goods[k], color=colors[k], label=col_titles[k], zorder=4);
        axs[0,i].fill_between(np.arange(5), goods[k]+goods_stds[k], goods[k]-goods_stds[k], color=colors[k], alpha = 0.15, zorder=4);
    ...
    axs[0,i].set_title(task, fontsize=15, fontweight='bold')
    axs[0,0].set_ylabel("MLE")
    axs[0,i].set_xlabel('Epochs')
    axs[0,i].set_xlim([0,4])
    axs[0,i].set_xticks(np.linspace(0,4,5))
    axs[0,i].set_xticklabels(['0','25','50','75','99'])
    if i==0 : axs[0,i].legend(loc='upper left');
    axs[0,i].axhline(y=0, linestyle='--', color='grey', zorder=2)
...
linspaces = [np.linspace(0,2,5), np.linspace(0,0.05,6), np.linspace(0,2,5)]
...
for i in range(3):
    ...
    if os.path.exists(f'/Volumes/GEADAH/3_Research/data/MIs/{task}_all_mis.npy'):
        ...
        mutual_informations = np.where(mutual_informations < 0, 0, mutual_informations)
        ns_2,ss_2, plot_mis_2 = mi_defs.plot_mi_contour(mutual_informations,d,1,num_init)
        CF = axs[1,i].contourf(ns_2,ss_2,plot_mis_2, 30, cmap='viridis')
        fig.colorbar(CF, ax=axs[1,i], location='bottom', shrink=0.9, aspect=15, ticks=linspaces[i])#, ticks=[0.2,0.6,1.0,1.4,1.8,2.2,2.6])

        # axs[2,i].set_title(r'$I(H_t,X_{\{t-1,t\}})$'))
    # axs[2,2].set_title('$d=0$, $n_i=2$')
    axs[1,i].set_xlabel('Gain $(n)$')
...
FILE = os.path.join(SAVEDIR,f'signals_fig')
